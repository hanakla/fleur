...
describe('MockOperationContext', () => {
    it('Example', async () => {
        ...
        class CountStore extends Store<{ count: number }> {
            ...
            private resetCount = listen(resetAction, () => this.updateWith(state => state.count = 0))
            private increaseCount = listen(increaseAction, (payload) => this.updateWith(state => state.count += payload.increase))
        ...
        const context = createMockOperationContext({
            stores: [
                mockStore(CountStore, { count: 100 })
            ]
        })

        // Spec
        await context.executeOperation(increaseOp, { increase: 100 })
        expect(context.dispatchs[0]).toMatchObject({ action: resetAction, payload: {} })
        expect(context.dispatchs[1]).toMatchObject({ action: increaseAction, payload: { increase: 10 } })
