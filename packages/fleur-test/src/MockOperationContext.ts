...
export class MockOperationContext {
    ...
    public async executeOperation<O extends Operation<any>>(operation: O, arg: OperationArg<O>): Promise<void> {
        await Promise.resolve(operation(this as any, arg))
    }

    public getStore<T extends StoreClass<any>>(StoreClass: T): InstanceType<T> {
        if (!StoreClass.storeName || StoreClass.storeName === '') throw new Error(`Store.storeName must be defined in ${Store.name}`)
    ...
    public dispatch<AI extends ActionIdentifier<any>>(action: AI, payload: ReturnType<AI>): void {
