...
describe('selector', () => {
  ...
  const selectFetchStatus = selector(getStore => getStore(TestStore).fetch)
  const selectUsers = selector(getStore => getStore(TestStore).users)
  ...
  it('Should type error passed extra arguments have selector', () => {
