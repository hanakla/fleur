...
import { ExtractStateOfStoreClass } from './Store'

type StateGetter = <T extends StoreClass<any>>(
  StoreClass: T,
...
type Selector = (getState: StateGetter, ...args: any[]) => any
...
export type SelectorResults<T extends readonly NonArgsWrappedSelector[]> = {
  [K in keyof T]: T[K] extends (...args: any) => infer R ? R : never
}

interface StateSelectorCreator {
...
/** Create selector with Store#state getter */
export const selector: StateSelectorCreator = (
  deps: Selector | NonArgsWrappedSelector[],
  ...
  return (getStore: StoreGetter, ...args: any[]) => {
    const getState = <T extends StoreClass<any>>(StoreClass: T) => {
      return getStore(StoreClass).state
    }
    return (deps as Selector)(getState, ...args)
  }
}
